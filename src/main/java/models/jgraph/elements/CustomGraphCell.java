...
public class CustomGraphCell extends DefaultGraphCell implements Cleanable, ModelOwner, CustomGraphElement {
	...
	public CustomGraphCell(DirectedGraphNode node, CustomGraphModel model, GraphLayoutConnection layoutConnection) {
		super(node.getLabel());
		this.node = node;
		this.model = model;
		...
		//GraphConstants.setOrientation(getAttributes(),node.getAttributeMap().get(AttributeMap.PREF_ORIENTATION, SwingConstants.NORTH));
	...
	@Override
	public CustomGraphPort addPort(Point2D offset, Object userObject) {
		CustomGraphPort port = new CustomGraphPort(userObject, model, layoutConnection);
		if (offset == null) {
			add(port);
		} else {
